...
public class Solution {
    ...
    private static double solve(String[] grid, int rabbitCount)
            throws Exception {
        ...
        List<List<RabbitLocation>> perms = chooseRabbitCells(emptyCells,
                rabbitCount);
    ...
    private static List<Cell[]> getCombos(List<Cell> items, int numSlots) {
    ...
    private static void setNextSlot(Cell[] combo, List<Cell> items,
            int startItemIndex, int numSlots, int startSlotIndex,
            List<Cell[]> result) {
        int slotIndex = startSlotIndex;
        int remainingSlots = numSlots - slotIndex - 1;

        if (!addCombo(combo, slotIndex, numSlots, result)) {
            for (int itemIndex = startItemIndex; itemIndex < items.size()
                    && remainingSlots <= (items.size() - itemIndex - 1); itemIndex++) {

                setSlot(combo, items, itemIndex, slotIndex);
                setNextSlot(combo, items, itemIndex + 1, numSlots,
                        slotIndex + 1, result);
            }
        }
    }
    ...
    private static Integer calcComponents(List<RabbitLocation> rabbitLocs,
            int[] neighbors) throws Exception {
        int components = 0;
        /*
         * for(int i=0; i<rabbitLocs.size(); i++){ RabbitLocation start =
         * rabbitLocs.get(i); if(!start.visited){ ++components; bfs(start); } }
         */
    ...
    private static RabbitLocation findClosestRabbit(int i,
            List<RabbitLocation> rabbitLocs, int[] neighbors) {
        ...
        for (int j = 0; j < rabbitLocs.size(); j++) {
            ...
            if (dist < minDist) {
                ...
                if (thisRabbit.location.x < closestRabbit.location.x) {
                    closestRabbit = thisRabbit;
                } else if (thisRabbit.location.x == closestRabbit.location.x
                        && thisRabbit.location.y < closestRabbit.location.y) {
                    closestRabbit = thisRabbit;
                }
            }
        }
